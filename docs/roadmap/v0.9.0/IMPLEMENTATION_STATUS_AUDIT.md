...
3. ~~**T11 指标只有注册没有打点**~~ → ✅ 已修复（数据面）：`operations_total`/`operation_step_duration_seconds`（WorkflowEngine）、`renewal_due`/`renewal_failures_total`/`certificate_seconds_to_expiry`（RenewalController）、`outbox_pending`（OutboxConsumer）、`challenge_cleanup_pending`（ChallengeCleanupScanner）、`deployment_total`（DeploymentOrchestrator）、`acme_requests_total`/`acme_request_duration_seconds`/`bad_nonce_total`（InstrumentedAcmeTransport）均有打点并有行为测试；新增 `src/server/metrics_endpoint.rs` 在 `[metrics].listen_addr`（默认 127.0.0.1:9090）暴露 `/metrics`。`repository_errors_total` 已打点（第四轮，13/13 指标全覆盖）；trace span 字段注入与 webhook 验签助手已落地（第五轮）。T11 剩余：`ca_id`/`challenge_type`/`provider_id`/`sink_id` 等字段的对应 span 侧注入（值在引擎记录上不存在，需在步骤执行器侧补）。
4. ~~**T04 EAB 为 stub**~~ → ✅ 已修复（2026-09-02 第四轮）：真实 HMAC-SHA256 EAB JWS + SecretRef 凭据。Account Key Rollover 已实现（第五轮）。仍缺：旧 `AcmeClient`/Manager 未按文档 facade 化，新旧两套 JWS 并存。
...
- **T06 传播策略配置 schema**：`[challenge.dns01.propagation]`（authoritative_quorum="all"|N、recursive_resolvers、recursive_quorum、initial/max_interval_secs）经 `PropagationSettings::to_policy()` 接入 `build_dns_presenter`；`propagation_timeout_secs` 首次真正被 worker 消费；QuorumSpec 自定义反序列化给出明确错误；示例配置更新；5 个新配置测试。
- **T11 `repository_errors_total` 打点**：`RepositorySet` 携带 `backend` 标签（memory/file）；引擎 `run_once`/`run_step` 与 OutboxConsumer 对逃逸的仓储错误递增 `{backend,error_class}`（新增 `storage` 粗类，投递失败不计入）；2 个确定性行为测试（破坏目录 → 指标出现且 delivery 调用数为 0）。至此 13 个最低指标全部有打点。
